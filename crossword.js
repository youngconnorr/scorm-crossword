...
function Crossw1rd(container_id) {
  ...
  this.drawGrid = function () {
    ...
    this.grid.attr(
      "style",
      "height:" + this.height * 28 + "px; width:" + this.width * 28 + "px;"
    );

    // add rows and cols and store cells in this.cells
    this.cells = [];
    for (let r = 0; r < this.height; r++) {
      let row = $('<div class="row"></div>').appendTo(this.grid);
      this.cells[r] = [];
      for (let c = 0; c < this.width; c++) {
        let cell = $('<div class="blank"></div>').appendTo(row);
        this.cells[r][c] = cell;
      }
    }
  ...
  this.drawClues = function () {
    let cluediv = $('<div class="clues"></div>').appendTo(this.container);
    cluediv.append('<h4 class="cluelabel">Across</h4>');
    let aol = $('<div class="across scroll-pane"></div>').appendTo(cluediv);
    cluediv.append('<h4 class="cluelabel">Down</h4>');
    let dol = $('<div class="down scroll-pane"></div>').appendTo(cluediv);
    for (let i = 0; i < this.clues.length; i++) {
      let clue = this.clues[i];
      let li;
      if (clue.d == "A") {
  ...
  this.clue_click = function (ev) {
    let clicked = $(this);
    let clue = self.clues[clicked.data("clueix")];
    self.direction = clue.d;
    let cell = self.cells[clue.y][clue.x];
    cell.click();
  ...
  this.activateClue = function (li) {
    ...
    let paddingMarginOffset = 8;
    let top =
      li.position().top - li.parent().position().top - paddingMarginOffset;
    self.container
  ...
  this.drawControls = function () {
    let div = $('<div class="controls"></div>').appendTo(this.container);
    let reset = $("<button>Reset</button>").appendTo(div);
  ...
  this.adjustDimensions = function () {
    ...
    let ctrlH = this.container.find(".controls").height();
  ...
  this.mapKeyBindings = function () {
    ...
    for (let i = 48; i <= 90; i++) {
      ...
      if ((i >= 48 && i <= 57) || (i >= 65 && i <= 90)) {
        $(document).bind("keypress", String.fromCharCode(i), function (e) {
          ...
          let c = String.fromCharCode(Crossw1rd.keyCode(e)).toUpperCase();
          let active = self.grid.find(".active");
    ...
    $(document).bind("keydown", "tab", function () {
      let cell = self.grid.find(".active");
      let curWordNum = self.wordNumber(cell);
      let nextWord = self.nextWord(curWordNum);
    ...
    $(document).bind("keydown", "shift+tab", function () {
      let cell = self.grid.find(".active");
      let curWordNum = self.wordNumber(cell);
      let prevWord = self.prevWord(curWordNum);
      self.activateCell(prevWord);
      return false;
    });
    // ctrl+shift+l - check letter
    $(document).bind("keydown", "shift+ctrl", function () {
      self.checkCell();
    });
  ...
  this.checkCell = function (cell) {
    ...
    let entered = cell.find(".letter").text();
    if (entered == "") return;
  ...
  this.checkWord = function () {
    let cell = this.grid.find(".active");
    let wordNum = this.wordNumber(cell);
    for (let i = 0; i < this.clues.length; i++) {
      let clue = this.clues[i];
  ...
  this.getState = function () {
    let delim = "|";
    let state = [delim]; // 1st char defines the row delimiter
    ...
    for (let i = 0; i < this.cells.length; i++) {
      ...
      for (let c = 0; c < row.length; c++) {
        ...
        if (!cell.hasClass("blank")) {
          let letter = cell.find(".letter").text();
          if (letter.length == 0) {
  ...
  this.resume = function (state) {
    let delim = state[0];
    let parts = state.split(delim);
    parts.shift(); // drop the 1st delimeter
    let id = parts.shift(); // get the ID
    let settingslen = parts.shift(); // the number of settings
    let settings = {};
    ...
    for (let i = 0; i < this.cells.length; i++) {
      let row = this.cells[i];
      let answers = parts.shift().split("");
      for (let c = 0; c < row.length; c++) {
        let cell = row[c];
        if (!cell.hasClass("blank")) {
          let letter = answers.shift();
          if (letter != " ") {
